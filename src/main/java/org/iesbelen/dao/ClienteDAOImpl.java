...
public class ClienteDAOImpl implements ClienteDAO {
    ...
    // Plantilla jdbc inyectada automáticamente por el framework Spring, gracias a
    // la anotación @Autowired.
    @Autowired
    private JdbcTemplate jdbcTemplate;

    /**
     * Inserta en base de datos el nuevo Cliente, actualizando el id en el bean
     * Cliente.
     */
    @Override
    ...
    public Optional<Cliente> find(int id) {
        ...
        Cliente fab = jdbcTemplate
                .queryForObject("SELECT * FROM cliente WHERE id = ?", (rs, rowNum) -> new Cliente(rs.getInt("id"),
                        rs.getString("nombre"),
                        rs.getString("apellido1"),
                        rs.getString("apellido2"),
                        rs.getString("ciudad"),
    ...
    public void update(Cliente cliente) {
        ...
        int rows = jdbcTemplate.update("""
                UPDATE cliente SET
                				...
                				categoría = ?
                		WHERE id = ?
                """, cliente.getNombre(), cliente.getApellido1(), cliente.getApellido2(), cliente.getCiudad(),
                cliente.getCategoria(), cliente.getId());
